...
#define BUTTON_LEFT_1  1
#define BUTTON_LEFT_2  2
#define BUTTON_RIGHT_1 3
#define BUTTON_RIGHT_2 4
...
far rom char str_preferences[] = "Preferences";
...
far rom char str_eject[] = "Eject";
far rom char str_change_card[] = "Change Card";
...
void ui_update_track(drive_state* drive)
{
	...
	n6610_draw_char(112, y, '0' + (drive->track / 10));
	n6610_draw_char(120, y, '0' + (drive->track % 10));
}
...
static void draw_emulation_drive_item(uint8_t drive, uint8_t selected)
{
	...
	n6610_set_font(FONT_8X14);
	n6610_draw_char(112, y, '0' + (wd179x_drive[drive].track / 10));
	n6610_draw_char(120, y, '0' + (wd179x_drive[drive].track % 10));
	n6610_draw_char(88, y, 'A' + drive);
	n6610_draw_char(96, y, ':');
...
static void draw_main_menu_item(ui_main_item item, uint8_t selected)
{
	...
	switch(item)
	{
	case ui_main_preferences:
		n6610_fill_area(0, 16, 130, 8);
		n6610_draw_rom_str(0, 16, str_preferences);
		break;
	case ui_main_reset:
...
static void draw_image_select_menu_item(uint8_t item, uint8_t selected)
{
	...
	if (item < file_count)
	{
		n6610_fill_area(0, 14 + item * 8, 130, 8);
		if (file_list[item].dir)
		{
			n6610_draw_char(0, 14 + item * 8, '/');
			n6610_draw_ram_str(8, 14 + item * 8, file_list[item].name);
		}
		else
			n6610_draw_ram_str(0, 14 + item * 8, file_list[item].name);
	}
	else if (item == FILE_LIST_SIZE)	
	{
		n6610_fill_area(0, 100, 130, 8);
		n6610_draw_rom_str(0, 100, str_eject);
	}
	else if (item == FILE_LIST_SIZE + 1)	
	{
		n6610_fill_area(0, 108, 130, 8);
		n6610_draw_rom_str(0, 108, str_change_card);
	}
}
...
{		
	...
	draw_image_select_menu_item(FILE_LIST_SIZE + 1, (selection == FILE_LIST_SIZE + 1));
}
...
{
	if (pressed_button == BUTTON_LEFT_1)
	{
	...
	if (pressed_button == BUTTON_LEFT_2)
	{
		draw_emulation_drive_item(selection, 0);
		selection ++;
		if (selection >= 4)
	...
	if (pressed_button == BUTTON_FIRE)
	{
		drive = selection;
		enter_image_select();
	}
	if (pressed_button == BUTTON_RIGHT_1)
	{
		/* Write Protect */
		wd179x_drive[selection].type_I_status ^= 0x40;
		draw_emulation_drive_item(selection, 1);
	}
	if (pressed_button == BUTTON_RIGHT_2)
		enter_main_menu();
}
...
static void handle_image_select(void)
{
	...
	if (pressed_button == BUTTON_FIRE)
	{
		if (selection == FILE_LIST_SIZE + 1)
		{
			card_initialized = 0;
			enter_image_select();
		}
		else if (selection == FILE_LIST_SIZE)
		{
			/* TODO: Eject */
		}
		else 
